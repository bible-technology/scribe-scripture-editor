...
// // //   const parsed = [];
// // //   let idCounter = 0;
// // //   let currentChapter = '1'; // Track current chapter during parsing
...
// // //       // Update current chapter when we encounter a chapter tag
// // //       if (fullTag === '\\c') {
...
// // //       if (fullTag === '\\p') {
// // //         if (content.trim()) {
// // //           // Improved regex to handle verses with various content including curly braces
// // //           const verseMatches = [...content.matchAll(/\\v\s+(\d+)\s+([^\\]*(?:\\(?!v\s+\d)[^\\]*)*)/g)];
// // //           if (verseMatches.length > 0) {
// // //             // First, add the \p tag itself
// // //             parsed.push({
// // //               id: idCounter++,
// // //               tag: '\\p',
// // //               content: '',
...
// // //               // Add any text before this \v as a standalone \p content
...
// // //               // Clean up the verse text (remove trailing dots, extra spaces, etc.)
// // //               const cleanVerseText = verseText.trim();

// // //               // Add the verse with chapter context
// // //               parsed.push({
// // //                 id: idCounter++,
// // //                 tag: '\\v',
// // //                 verseNumber,
...
// // //                 visible: true,
// // //                 chapter: currentChapter, // Include chapter context
// // //               });

// // //               lastIndex = match.index + full.length;
// // //             }

// // //             // Add any trailing content as a \p
...
// // //       }
// // //     } else if (line.trim()) {
// // //       // Non-tagged content (maybe malformed or prose)
// // //       parsed.push({
// // //         id: idCounter++,
// // //         tag: null,
// // //         content: line,
...
// // //       // console.log('Parsed USFM:', parsedUSFM.filter(p => p.visible && p.tag === '\\v'));
// // //     }
// // //   }, [usfmString]);
...
// // //   // Helper function to find the preceding verse for any element
// // //   const findPrecedingVerse = (currentId) => {
// // //     const editableElements = parsed.filter(({ tag, visible }) => visible && tag !== '\\c');
// // //     const currentIndex = editableElements.findIndex(el => el.id === currentId);

// // //     if (currentIndex === -1) return null;

// // //     // Look backwards from current element to find the most recent verse
...
// // //   // Helper function to update scrRef based on element focus
// // //   const updateScrRefForElement = (elementId) => {
...
// // //       // If it's a verse element, use it directly
// // //       targetVerse = {
...
// // //     } else {
// // //       // For non-verse elements, find the preceding verse
...
// // //       setDebugInfo(`Updated scrRef: ${newScrRef.bookCode} ${newScrRef.chapterNum}:${newScrRef.verseNum} (from element: ${currentElement.tag || 'text'})`);
// // //     }
// // //   };

// // //   const scrollToReference = (ref) => {
// // //     if (!ref || !ref.chapterNum || !ref.verseNum) {
// // //       setDebugInfo('Invalid reference: ' + JSON.stringify(ref));
// // //       return;
// // //     }
...
// // //     setDebugInfo(`Looking for Chapter ${chapterNum}, Verse ${verseNum}`);

// // //     // Find the verse in parsed data using the stored chapter context
...
// // //     if (!foundVerse) {
// // //       setDebugInfo(`Verse not found: Chapter ${chapterNum}, Verse ${verseNum}. Available verses: ${parsed.filter(p => p.tag === '\\v').map(v => `${v.chapter}:${v.verseNumber}`).join(', ')}`);
// // //       setIsScrolling(false);
// // //       return;
// // //     }

// // //     setDebugInfo(`Found verse: ${foundVerse.verseNumber} in chapter ${foundVerse.chapter}`);

// // //     const verseKey = `${chapterNum}-${verseNum}`;
// // //     const verseElement = verseRefs.current[verseKey];

// // //     if (verseElement) {
// // //       setDebugInfo(`Scrolling to verse element: ${verseKey}`);
...
// // //         }
// // //         setIsScrolling(false);
// // //         setDebugInfo(`Successfully scrolled to ${verseKey}`);
// // //       }, 200);
// // //     } else {
// // //       setDebugInfo(`Verse element not found in DOM: ${verseKey}. Available refs: ${Object.keys(verseRefs.current).join(', ')}`);
// // //       const chapterElement = chapterRefs.current[chapterNum];
// // //       if (chapterElement) {
// // //         setDebugInfo(`Falling back to chapter scroll: ${chapterNum}`);
...
// // //     //         setCurrentFocusId(editableIds[prevIndex]);
// // //     //         updateScrRefForElement(editableIds[prevIndex]);
// // //     //       });
// // //     //     }
// // //     //   }
// // //     // }
...
// // //           e.preventDefault();
// // //           const nextIndex = currentIndex + 1;
// // //           const nextElement = contentRefs.current[editableIds[nextIndex]];
// // //           if (nextElement) {
// // //             requestAnimationFrame(() => {
// // //               nextElement.focus();
// // //               const range = document.createRange();
...
// // //               sel.removeAllRanges();
// // //               sel.addRange(range);
// // //               setCurrentFocusId(editableIds[nextIndex]);
// // //               updateScrRefForElement(editableIds[nextIndex]);
// // //             });
// // //           }
// // //         }
// // //       };
...
// // //       if (e.key === 'ArrowLeft') {
// // //         if (isRTL) {
// // //           // ArrowLeft in RTL: moves forward (next element)
// // //           if (cursorPos === textLength) moveNext();
// // //         } else {
// // //           // ArrowLeft in LTR: moves backward (prev element)
// // //           if (cursorPos === 0) movePrev();
// // //         }
// // //       } else if (e.key === 'ArrowRight') {
// // //         if (isRTL) {
// // //           // ArrowRight in RTL: moves backward (prev element)
// // //           if (cursorPos === 0) movePrev();
// // //         } else {
// // //           // ArrowRight in LTR: moves forward (next element)
// // //           if (cursorPos === textLength) moveNext();
// // //         }
// // //       }
// // //     }
...
// // //         color: '#666'
// // //       }}>
// // //         <strong>Navigation:</strong> Use arrow keys to move between elements. Chapter and verse numbers are protected from deletion.
// // //         {scrRef && (
// // //           <span style={{ marginLeft: '20px', color: '#007bff' }}>
// // //             <strong>Current Reference:</strong> {scrRef.bookCode} {scrRef.chapterNum}:{scrRef.verseNum}
// // //           </span>
// // //         )}
// // //         {selectedVerseId && (
// // //           <span style={{ marginLeft: '20px', color: '#EE5A24' }}>
// // //             <strong>Selected Verse</strong> (highlighted in orange)
// // //           </span>
// // //         )}
// // //         {isScrolling && (
...
// // //                     // console.log(`Registered chapter ref: ${content}`);
// // //                   }
// // //                 }}
// // //                 style={{
// // //                   textAlign: 'center',
...
// // //               //       // console.log(`Registered verse ref: ${verseKey} for verse ${verseNumber} in chapter ${chapter}`);
// // //               //     }
// // //               //   }}
// // //               //   style={{
// // //               //     display: 'flex',
...
// // //               //   }}
// // //               // >
// // //               <div key={id} ref={(el) => { if (el) { verseRefs.current[verseKey] = el; } }}
// // //                 style={{
// // //                   display: 'flex',
// // //                   alignItems: 'center',
// // //                   marginBottom: 10,
...
// // //                     // border: currentFocusId === id ? '2px solid #4caf50' : '2px solid transparent',
...
// //       // Set saved state after a short delay to show the user the save completed
// //       setTimeout(() => {
// //         setIsDirty(false);
// //         setIsSaving(false);
// //       }, 500);
...
// //     // Set dirty state and trigger save if content changed
// //     if (hasChanged) {
// //       setIsDirty(true);
// //       // Use setTimeout to ensure state is updated before triggering save
// //       setTimeout(() => {
// //         triggerSave();
// //       }, 0);
// //     }
...
// NOTE: The saveToFile import is commented out as the implementation was not provided.
...
const USFMEditor = ({
  ...
  bookId,
  filePath,
  onUsfmChange, // This prop is crucial for saving
}) => {
  const [parsed, setParsed] = useState([]);
  const [filename, setFilename] = useState('');
  const [currentFocusId, setCurrentFocusId] = useState(null);
  ...
  const cursorPosRef = useRef(null); // Ref to store cursor position
  ...
  useEffect(() => {
    ...
    if (content) {
      ...
      setParsed(parsedUSFM);
      setFilename(usfmString ? 'FromProps.usfm' : 'ProvidedData.usfm');
      setIsDirty(false); // Reset dirty state on new content
  ...
  const moveToElement = useCallback(
    (targetIndex, cursorPosition = 'start') => {
      ...
      if (targetIndex < 0 || targetIndex >= editableElements.length)
        return;
      const nextId = editableElements[targetIndex];
      const nextElement = contentRefs.current[nextId];
      if (nextElement) {
        nextElement.focus();
        setCursor(nextElement, cursorPosition);
        setCurrentFocusId(nextId);
        updateScrRefDebounced(nextId);
      }
    },
  ...
  const handleKeyDown = useCallback(
    (e, id) => {
      ...
      if (e.key === 'ArrowUp') {
        ...
        if (
          textDirection === 'rtl'
            ? isCursorAtBoundary(currentElement, 'end')
            : isCursorAtBoundary(currentElement, 'start')
        ) {
          e.preventDefault();
          moveToElement(currentIndex - 1, 'end');
        }
    ...
    [isScrolling, editableElements, moveToElement, textDirection],
  );

  /**
   * ---- DUAL AUTO-SAVE LOGIC ----
   */

  // Debounced function for saving to localStorage (3 seconds)
  const debouncedLocalSave = useMemo(
    () =>
      debounce((currentParsed, key) => {
        if (!key || !isDirtyRef.current) return;
        console.log('Saving backup to localStorage...');
        const usfmString = currentParsed
          .map((line) => line.original)
          .join('\n');
        localStorage.setItem(key, usfmString);
      }, 1000),
    [],
  );

  // Debounced function for saving to file system (5 seconds)
  const debouncedFileSave = useMemo(
    () =>
      debounce((currentParsed, usfmChangeHandler) => {
        if (!usfmChangeHandler || !isDirtyRef.current) return;

        console.log('Auto-saving to file...');
        setIsSaving(true);

        const usfmString = currentParsed
          .map((line) => line.original)
          .join('\n');
        const saveResult = usfmChangeHandler(usfmString);
        ...
        Promise.resolve(saveResult)
          .then(() => {
            ...
            localStorage.removeItem(
              `usfm-editor-backup-${filePath || bookId || 'default'
              }`,
            );
            console.log;
            ('Auto-save to file completed successfully');
          })
          .catch((error) =>
  ...
  // This single effect triggers both save handlers
  useEffect(() => {
    if (isDirty) {
      const backupKey = `usfm-editor-backup-${filePath || bookId || 'default'
        }`;
      debouncedLocalSave(parsed, backupKey);
      debouncedFileSave(parsed, onUsfmChange);
    }
    return () => {
      debouncedLocalSave.cancel();
      debouncedFileSave.cancel();
    };
  }, [
    parsed,
    isDirty,
  ...
  // ---- END OF DUAL AUTO-SAVE LOGIC ----

  useEffect(() => {
    if (
      cursorPosRef.current &&
      contentRefs.current[cursorPosRef.current.id]
    ) {
      const { id, start, end } = cursorPosRef.current;
