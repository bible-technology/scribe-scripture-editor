...
const Player = ({
	...
	const [volume, setVolume] = useState(
		Number(localStorage.getItem(LS_AUDIO_VOLUME_KEY)) &&
			typeof Number(
				localStorage.getItem(LS_AUDIO_VOLUME_KEY) === 'number',
			)
			? Number(localStorage.getItem(LS_AUDIO_VOLUME_KEY))
			: 0.5,
	);
	...
	const milliseconds =
		playTime > 0
			? Math.floor((playTime - Math.floor(playTime)) * 100)
			: time > 0
			? time % 100
			: 0;
	...
	const handleKeyPress = useCallback(
		...
		[trigger],
	); // ---> change to space for play and pause
