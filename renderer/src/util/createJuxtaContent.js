...
export const createJuxtaContent = (
  ...
  return new Promise(async (resolve) => {
    ...
    logger.debug('createJuxtaContent.js', 'Creating the story md files');
    ...
    //     logger.debug('createJuxtaContent.js', `${currentFileName} is been Imported`);
    ...
    //   } else {
    //     logger.debug('createJuxtaContent.js', 'Creating the md file using RCL function JsonToMd');
    //     const file = JsonToMd(storyJson, '');
    //     const fs = window.require('fs');
    //     if (!fs.existsSync(folder)) {
    //       fs.mkdirSync(folder, { recursive: true });
    //     }
    //     logger.debug('createJuxtaContent.js', 'Writing File to the Content Directory');
    ...
    // logger.debug('createJuxtaContent.js', 'Creating OBS front and back md file in content');
    ...
    //   logger.debug('createJuxtaContent.js', `${fileFront.name} is been Imported`);
    // } else {
    //   fileFront.name = 'front.md';
    //   fileFront.content = OBSFront;
    //   logger.debug('createJuxtaContent.js', `${fileFront.name} default is created`);
    // }
    // fs.writeFileSync(path.join(folder, fileFront.name), fileFront.content);
    // let stats = fs.statSync(path.join(folder, fileFront.name));
    ...
    //   logger.debug('createJuxtaContent.js', `${fileBack.name} has been Imported`);
    // } else {
    //   fileBack.name = 'back.md';
    //   fileBack.content = OBSBack;
    //   logger.debug('createJuxtaContent.js', `${fileBack.name} default is created`);
    // }
    // fs.writeFileSync(path.join(folder, fileBack.name), fileBack.content);
    // stats = fs.statSync(path.join(folder, fileBack.name));
    ...
    await books.forEach((book) => {
      if (bookAvailable(importedFiles, book)) {
        ...
        fs.writeFileSync(path.join(folder, 'license.md'), copyright.licence);
    ...
    logger.debug('createJuxtaContent.js', 'in Edit obs content files');
    importedFiles.forEach((file) => {
      if (file.id !== 'front.md' && file.id !== 'back.md') {
        logger.debug('createJuxtaContent.js', `${file.id} is been Imported`);
        ...
        const mimeType = file.id === 'front.md' ? 'application/json' : 'application/json';
  ...
  export const createWebObsContent = (
    ...
    call,
      ...
      const uploadFileToSupabase = async (filePath, fileContent) => {
        const { data: file, error } = await sbStorageUpload(filePath, new Blob([fileContent], { type: 'application/json' }), {
            upsert: true,
          });
