...
const userPath = async (window) => {
  const path = await window.evaluate(() => ({ ...window.localStorage }));
  return path.userPath;
};

// Retrieves and parses a JSON file containing user information
export const userJson = async (window, packageInfo, path) => path.join(await userPath(window), packageInfo.name, 'users', 'users.json');

// Constructs the path to a user's folder.
export const userFolder = async (window, userName, packageInfo, path) => path.join(await userPath(window), packageInfo.name, 'users', userName.toLowerCase());
